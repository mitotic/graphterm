...
GTWebSocket.prototype.onmessage = function(evt) {
	...
	for (var j=0; j<payload_obj.length; j++) {
		...
		if (cmd_type == "alert") {
		    ...
		    if (!gNotebook) {
			gNotebook = new GTNotebook(newElem, gPromptIndex)
		    }
		    gNotebook.addCell(cellIndex, cellType, beforeCellIndex);
...
function AjaxKeypress(evt) {
    ...
    if (gNotebook && k == String.fromCharCode(4) && window.confirm("Exit notebook mode?")) {
	// Ctrl-D: close notebook
	GTCloseNotebook();
	return false;
